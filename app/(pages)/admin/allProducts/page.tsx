...
} from "@/app/redux/features/adminProductsSlice";
...
function parseProductsPayload(payload: unknown): AdminProduct[] {
...
function parseCategoriesPayload(payload: unknown): CategoryOption[] {
  const envelope = payload as ApiEnvelope<unknown>;
  if (!envelope?.success || !Array.isArray(envelope.data)) {
...
async function fetchAllProductsSnapshot(): Promise<AdminProduct[]> {
  let page = 1;
  let totalPages = 1;
...
async function fetchActiveCategories(): Promise<CategoryOption[]> {
  const params = new URLSearchParams({
    status: "ACTIVE",
    page: "1",
    pageSize: String(API_PAGE_SIZE),
...
function normalizeImagesInput(input: string): string[] {
  return Array.from(
    new Set(
      input
        .split(/[\n,]+/)
...
function parseNumericField(raw: string, field: string): number {
